...
length(unique(bsResiDat$seqClust50[apply(tag23, 1, any)]))
length(unique(bsResiDat$seqClust50[apply(tag23, 1, any)])) / length(unique(bsResiDat$seqClust50))

length(unique(bsResiDat$seqClust50[apply(tag26, 1, any)]))
length(unique(bsResiDat$seqClust50[apply(tag26, 1, any)])) / length(unique(bsResiDat$seqClust50))

# number of interactions
apply(tag23, 2, sum)
apply(tag26, 2, sum)
sum(apply(tag23, 2, sum))
sum(apply(tag26, 2, sum))
...
sum(cuWeights[apply(tag23, 1, any)])

for (i in 1:ncol(tag26)){
  cat(colnames(tag26)[i], '\t')
  cat(sum(cuWeights[tag26[,i]]), '\n')
}
sum(cuWeights[apply(tag26, 1, any)])


# shared clusters?
sum(unique(bsResiDat$seqClust50[apply(tag23, 1, any)]) %in% unique(bsResiDat$seqClust50[apply(tag26, 1, any)]))
